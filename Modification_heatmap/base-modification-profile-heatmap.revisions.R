...
## load table
base_mod <- read.csv("tRNA-base-modification-average-isodecoders.txt", sep = "\t", header = T)

##LFC: add Isodecoder family and compartment to Sprinzl table
sprinzl$Compartment <- sprinzl$GeneName %>% str_replace("-.*", "")
sprinzl$Isodecoder <- sprinzl$GeneName %>% str_replace(".*Ath-", "") %>% str_replace("-.*", "")

##LFC: select useful columns 
sprinzl <- sprinzl %>% ungroup() %>% select(Isodecoder, Compartment, SprinzlPos, Region, letter) %>% unique()
...
## LFC: I adjusted the 3 following lines for Isodecoder families analysis
...
base_mod$Compartment <- base_mod$Compartment %>% str_replace("plastid", "chloroplast")
...
##LFC: select useful columns 
base_mod <- base_mod %>% select(Isodecoder, Compartment, SprinzlPos, letter, base_mod_total_avg) 

## re-shape dataframe to have genes as rows and sprinzl positions as columns, filled by base modification percentages
##LFC: I adjusted the following line for Isodecoder families analysis
data <- dcast(base_mod, Isodecoder + Compartment + SprinzlPos ~ letter, value.var = "base_mod_total_avg") 
data[is.na(data)] <- 0

##LFC: I think this was the best way to avoid downstream modificiations of the code
data$Comp_Iso <- paste(data$Compartment, data$Isodecoder, sep = "_")
data <- data %>% select(Comp_Iso, Compartment, SprinzlPos:(ncol(data)-1))
...
## LFC: I adjusted the next 2 lines for Isodecoder families analysis
gene_col$Genome <- gene_col$Comp_Iso %>% str_replace("_.*", "")
gene_col<- gene_col %>% column_to_rownames('Comp_Iso')
...
##LFC: I adjusted the following line for Isodecoder families analysis
collapsed_data <- collapsed_data %>% column_to_rownames('Comp_Iso')
