...
## DBS: I edited this to make threshold based on SUM of all misincorporations
## DBS: I also reduced the threshold to only 0.5%. This ensures that all the refs with sufficient coverage get at least one variant call. Otherwise, they don't show up on the map.
thsd <- 0.5
base_mod <- base_mod %>%
  filter(
    (Reference == "A" & (`C_perc` + `G_perc` + `T_perc` + `Del_perc` >= thsd)) |
    (Reference == "C" & (`A_perc` + `G_perc` + `T_perc` + `Del_perc` >= thsd)) |
    (Reference == "G" & (`A_perc` + `C_perc` + `T_perc` + `Del_perc` >= thsd)) |
    (Reference == "T" & (`A_perc` + `C_perc` + `G_perc` + `Del_perc` >= thsd))
  )
